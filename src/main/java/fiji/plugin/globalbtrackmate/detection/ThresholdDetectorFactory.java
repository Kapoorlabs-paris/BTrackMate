...
import static fiji.plugin.globalbtrackmate.detection.DetectorKeys.DEFAULT_TARGET_CHANNEL;
...
import static fiji.plugin.globalbtrackmate.io.IOUtils.writeTargetChannel;
...
import fiji.plugin.globalbtrackmate.gui.components.detector.ThresholdDetectorConfigurationPanel;
...
public class ThresholdDetectorFactory<T extends RealType<T> & NativeType<T>> implements SpotDetectorFactory<T> {
	...
	public static final String INFO_TEXT = "<html>" + "This detector creates spots by thresholding a grayscale image."
			+ "<p>" + "Pixels in the designated channel that have "
			...
			+ "spot is created for each region in its center, with a volume equal to the " + "region volume."
			+ "</html>";
	...
	public SpotDetector<T> getDetector(final Interval interval, final int frame) {
		...
		final ThresholdDetector<T> detector = new ThresholdDetector<>(imFrame, interval, calibration,
				intensityThreshold, simplifyContours);

...
import static fiji.plugin.globalbtrackmate.gui.Icons.EXECUTE_ICON;
...
import fiji.plugin.globalbtrackmate.action.CaptureOverlayAction;
import fiji.plugin.globalbtrackmate.action.TrackMateAction;
...
public class ActionChooserPanel extends ModuleChooserPanel<TrackMateActionFactory> {
	...
	public ActionChooserPanel(final ActionProvider actionProvider, final TrackMate btrackmate,
					...
					public void run() {
						try {
							...
							if (null == action) {
								...
								action.execute(btrackmate, selectionModel, displaySettings,

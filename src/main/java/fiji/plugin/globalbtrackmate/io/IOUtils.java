...
import static fiji.plugin.globalbtrackmate.detection.DetectorKeys.KEY_DOWNSAMPLE_FACTOR;
import static fiji.plugin.globalbtrackmate.detection.DetectorKeys.KEY_DO_MEDIAN_FILTERING;
import static fiji.plugin.globalbtrackmate.detection.DetectorKeys.KEY_DO_SUBPIXEL_LOCALIZATION;
import static fiji.plugin.globalbtrackmate.detection.DetectorKeys.KEY_RADIUS;
import static fiji.plugin.globalbtrackmate.detection.DetectorKeys.KEY_TARGET_CHANNEL;
import static fiji.plugin.globalbtrackmate.detection.DetectorKeys.KEY_THRESHOLD;
...
public class IOUtils {
	 ...
	 *
	 * @param file   a default file, will be used to display a default choice in the
	 *               file chooser.
	 * @param parent the {@link Frame} to lock on this dialog. It can be
	 ...
	 * substitute a default value of 0 if the attribute is not found or of the wrong
	 * type.
	 */
	...
	public static final int readIntAttribute(final Element element, final String name, final Logger logger,
		...
		if (null == att) {
			logger.error("Could not find attribute " + name + " for element " + element.getName()
					+ ", substituting default value: " + defaultValue + ".\n");
			return val;
		}
		try {
			val = att.getIntValue();
		} catch (final DataConversionException e) {
			logger.error("Cannot read the attribute " + name + " of the element " + element.getName()
					+ ", substituting default value: " + defaultValue + ".\n");
		}
		return val;
	}
	...
	public static final boolean readDoubleAttribute(final Element element, final Map<String, Object> settings,
	...
	public static final boolean readIntegerAttribute(final Element element, final Map<String, Object> settings,
			final String parameterKey, final StringBuilder errorHolder) {
		final String str = element.getAttributeValue(parameterKey);
		if (null == str) {
			//errorHolder.append("Attribute " + parameterKey + " could not be found in XML element.\n");
	...
	public static final boolean readBooleanAttribute(final Element element, final Map<String, Object> settings,
			final String parameterKey, final StringBuilder errorHolder) {
		final String str = element.getAttributeValue(parameterKey);
		if (null == str) {
			//errorHolder.append("Attribute " + parameterKey + " could not be found in XML element.\n");
	...
	public static final boolean readStringAttribute(final Element element, final Map<String, Object> settings,
			final String parameterKey, final StringBuilder errorHolder) {
		final String str = element.getAttributeValue(parameterKey);
		if (null == str) {
			//errorHolder.append("Attribute " + parameterKey + " could not be found in XML element.\n");
	...
	/**
	 * Unmarshall the attributes of a JDom element in a map of doubles. Mappings are
	 * <b>added</b> to the specified map. If a value is found not to be a double, an
	 * error is returned.
	 *
	 ...
	 */
	public static boolean unmarshallMap(final Element element, final Map<String, Double> map,
		...
		for (final Attribute att : attributes) {
			...
			try {
				...
				errorHolder
						.append("Could not convert the " + key + " attribute to double. Got " + att.getValue() + ".\n");
				ok = false;
			}
		}
		return ok;
	...
	public static final boolean writeTargetChannel(final Map<String, Object> settings, final Element element,
			final StringBuilder errorHolder) {
		return writeAttribute(settings, element, KEY_TARGET_CHANNEL, Integer.class, errorHolder);
	}

	public static final boolean writeRadius(final Map<String, Object> settings, final Element element,
			final StringBuilder errorHolder) {
		return writeAttribute(settings, element, KEY_RADIUS, Double.class, errorHolder);
	}

	public static final boolean writeThreshold(final Map<String, Object> settings, final Element element,
			final StringBuilder errorHolder) {
		return writeAttribute(settings, element, KEY_THRESHOLD, Double.class, errorHolder);
	}

	public static final boolean writeDoMedian(final Map<String, Object> settings, final Element element,
			final StringBuilder errorHolder) {
		return writeAttribute(settings, element, KEY_DO_MEDIAN_FILTERING, Boolean.class, errorHolder);
	}

	public static final boolean writeDoSubPixel(final Map<String, Object> settings, final Element element,
			final StringBuilder errorHolder) {
		return writeAttribute(settings, element, KEY_DO_SUBPIXEL_LOCALIZATION, Boolean.class, errorHolder);
	}

	public static final boolean writeDownsamplingFactor(final Map<String, Object> settings, final Element element,
			final StringBuilder errorHolder) {
		return writeAttribute(settings, element, KEY_DOWNSAMPLE_FACTOR, Integer.class, errorHolder);
	}

	/**
	 * Add a parameter attribute to the given element, taken from the given settings
	 * map. Basic checks are made to ensure that the parameter value can be found
	 * and is of the right class.
	 * 
	 ...
	 * @return <code>true</code> if the parameter was found, of the right class, and
	 *         was successfully added to the element, <code>false</code> if not, and
	 *         updated the specified error holder.
	 */
	public static final boolean writeAttribute(final Map<String, Object> settings, final Element element,
			final String parameterKey, final Class<?> expectedClass, final StringBuilder errorHolder) {
		...
		if (!expectedClass.isInstance(obj)) {
			errorHolder.append("Expected " + parameterKey + " parameter to be a " + expectedClass.getName()
					+ " but was a " + obj.getClass().getName() + ".\n");
			return false;
		}

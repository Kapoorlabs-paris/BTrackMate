...
import static fiji.plugin.globalbtrackmate.gui.Icons.EDIT_SETTINGS_ICON;
...
import fiji.plugin.globalbtrackmate.gui.displaysettings.ConfigTrackMateDisplaySettings;
 ...
 * {@link fiji.plugin.globalbtrackmate.visualization.AbstractTrackMateModelView}. This
...
public class ConfigureViewsPanel extends JPanel {
	...
	public ConfigureViewsPanel(final DisplaySettings ds, final FeatureDisplaySelector featureSelector,
			final String spaceUnits, final Action launchTrackSchemeAction, final Action showTrackTablesAction,
		...
		final JFrame editor = ConfigTrackMateDisplaySettings.editor(ds,
				"Configure the display settings used in this current session.", "TrackMate display settings");
		...
		cmbboxTrackDisplayMode.addActionListener(
				e -> ds.setTrackDisplayMode((TrackDisplayMode) cmbboxTrackDisplayMode.getSelectedItem()));
		...
		final SpinnerNumberModel numberModelDrawingZDepth = new SpinnerNumberModel(ds.getZDrawingDepth(), 0.5, 5000.,
				0.5);
		...
		ftfSpotRadius.addPropertyChangeListener("value",
				e -> ds.setSpotDisplayRadius(((Number) ftfSpotRadius.getValue()).doubleValue()));

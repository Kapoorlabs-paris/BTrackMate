...
import fiji.plugin.globalbtrackmate.TrackMatePlugIn;
...
import fiji.plugin.globalbtrackmate.gui.wizard.BTrackMateWizardSequence;
...
public class TrackMatePlugIn implements PlugIn {
	...
	public void run(final String imagePath) {
		...
		final TrackMateModelView displayer = new HyperStackDisplayer(model, selectionModel, globalimp,
	...
	protected static WizardSequence PseudocreateSequence(final TrackMate btrackmate,
			final SelectionModel selectionModel, final DisplaySettings displaySettings, final Boolean secondrun) {
		return new BTrackMateWizardSequence(btrackmate, selectionModel, displaySettings, secondrun);
	}
...
//		new TrackMatePlugIn().run( "samples/Merged.tif" );
		new TrackMatePlugIn().run("/Users/aimachine/Downloads/CellTracking/merged.tif");

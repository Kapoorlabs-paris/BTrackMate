...
import fiji.plugin.globalbtrackmate.features.spot.SpotMorphologyAnalyzerFactory;
import fiji.plugin.globalbtrackmate.gui.components.FeatureDisplaySelector;
import fiji.plugin.globalbtrackmate.gui.components.FilterGuiPanel;
...
import fiji.plugin.globalbtrackmate.util.EverythingDisablerAndReenabler;
...
public class SpotFilterDescriptor extends WizardPanelDescriptor {
	...
	public SpotFilterDescriptor(final TrackMate trackmate, final List<FeatureFilter> filters,
			final FeatureDisplaySelector featureSelector) {
		super(KEY);
		this.trackmate = trackmate;
		final FilterGuiPanel component = new FilterGuiPanel(trackmate.getModel(), trackmate.getSettings(),
				TrackMateObject.SPOTS, filters, Spot.QUALITY, featureSelector);
	...
	public Runnable getForwardRunnable() {
		return new Runnable() {
			@Override
			public void run() {
				final EverythingDisablerAndReenabler disabler = new EverythingDisablerAndReenabler(
						(Container) targetPanel, new Class[] { JLabel.class });
				disabler.disable();
				try {
					...
					if (trackmate.getSettings().detectorFactory != null
						...
						final SpotMorphologyAnalyzerProvider spotMorphologyAnalyzerProvider = new SpotMorphologyAnalyzerProvider(
								settings.imp.getNChannels());
						@SuppressWarnings("rawtypes")
						final List<SpotMorphologyAnalyzerFactory> factories = spotMorphologyAnalyzerProvider.getKeys()
								.stream().map(key -> spotMorphologyAnalyzerProvider.getFactory(key))

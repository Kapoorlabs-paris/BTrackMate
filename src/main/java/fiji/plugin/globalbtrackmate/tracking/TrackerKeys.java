...
public class TrackerKeys {
	...
	/**
	 * Key for the parameter specifying the maximal linking distance. The expected
	 * value must be a Double and should be expressed in physical units.
	 */
	 ...
	 * particles. Expected values should be a <code>Map&lt;String, Double&gt;</code>
	 * where the map keys are spot feature names.
	 * 
	 * @see fiji.plugin.globalbtrackmate.Spot#getFeature(String)
	 */
	 ...
	 * when detecting gap closing. Expected values are {@link Integer}s greater than
	 * 0. A value of 1 means that a detection might be missed in 1 frame, and the
	 * track will not be broken. And so on.
	 */
	 ...
	 * values are {@link Double}s and should be expressed in physical units. If two
	 * spots, candidate for a gap-closing event, are found separated by a distance
	 * larger than this parameter value, gap-closing will not occur.
	 */
	 ...
	 * <code>Map&lt;String, Double&gt;</code> where the map keys are spot feature
	 * names.
	 * 
	 * @see fiji.plugin.globalbtrackmate.Spot#getFeature(String)
	 */
	...
	/**
	 * Key for the parameter specifying whether we allow the detection of merging
	 * events. Expected values are {@link Boolean}s.
	 */
	 ...
	 * are {@link Double}s and should be expressed in physical units. If two spots,
	 * candidate for a merging event, are found separated by a distance larger than
	 * this parameter value, track merging will not occur.
	 */
	...
	/**
	 * Key for the parameter specifying whether we allow the detection of splitting
	 * events. Expected values are {@link Boolean}s.
	 */
	 ...
	 * are {@link Double}s and should be expressed in physical units. If two spots,
	 * candidate for a merging event, are found separated by a distance larger than
	 * this parameter value, track splitting will not occur.
	 */
	...
	/**
	 * Key for the cutoff percentile parameter. Expected values are {@link Double}s.
	 */
	...
	/**
	 * Key for the parameter that stores the blocking value: cost for non-physical,
	 * forbidden links. Expected values are {@link Double}s, and are typically very
	 * large.
	 */

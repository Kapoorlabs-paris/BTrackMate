package fiji.plugin.globalbtrackmate.util;
...
public class FileChooser {
	...
	public static File chooseFile(final Component parent, final String selectedFile, final FileFilter fileFilter,
			final String dialogTitle, final DialogType dialogType) {
		return chooseFile(parent, selectedFile, fileFilter, dialogTitle, dialogType, SelectionMode.FILES_ONLY);
	}

	public static File chooseFile(final Component parent, final String selectedFile, final FileFilter fileFilter,
			final String dialogTitle, final DialogType dialogType, final SelectionMode selectionMode) {
		return chooseFile(useJFileChooser, parent, selectedFile, fileFilter, dialogTitle, dialogType, selectionMode);
	}

	public static File chooseFile(boolean useJFileChooser, final Component parent, final String selectedFile,
			final FileFilter fileFilter, final String dialogTitle, final DialogType dialogType,
		 ...
		 *
		 * Otherwise, use "Open" or "Save", depending on DialogType. If a FileFilter is
		 * provided, append the FileFilter description, leading to "Open xml files" or
		 * similar.
		 */
		...
		if (useJFileChooser) {
			...
			final int returnVal = isSaveDialog ? fileChooser.showSaveDialog(parent)
		...
		} else // use FileDialog
		{
			...
			/*
			 * If provided parent is a Frame or a Dialog, we can use it. Otherwise use null
			 * as parent.
			 */
			final FileDialog fd;

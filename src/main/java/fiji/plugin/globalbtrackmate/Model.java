...
public class Model {
	 ...
	 * needs to be configured with modification spot and edge targets, so it uses a
	 * different system (see {@link #flushUpdate()}).
	 */
	 ...
	 *
	 * @param doNotify if <code>true</code>, model listeners will be notified with a
	 ...
	 *
	 * @param doNotify if true, will file a {@link ModelChangeEvent#SPOTS_COMPUTED}
	 *                 event.
	 ...
	 * @param doNotify    if true, will file a
	 ...
	 * have been computed. This method serves as a manual trigger for this event.
	 */
	 ...
	 * then mark it for feature update. If the source spot could not be found in the
	 * source frame, nothing is done and <code>null</code> is returned.
	 * <p>
	...
	public synchronized Spot moveSpotFrom(final Spot spotToMove, final Integer fromFrame, final Integer toFrame) {
		...
		if (DEBUG) {
			System.out.println(
					"[TrackMateModel] Moving " + spotToMove + " from frame " + fromFrame + " to frame " + toFrame);
		}
	 ...
	 * </pre>
	 * 
	 * @param spotToAdd the spot to add.
	 * @param toFrame   the frame to add it to.
	 *
	...
	/**
	 * Removes a single spot from the collections managed by this model. If the spot
	 * cannot be found, nothing is done and <code>null</code> is returned.
	 * <p>
	 * For the model update to happen correctly and listeners to be notified
	 * properly, a call to this method must happen within a transaction, as in:
	 *
	...
	/**
	 * Mark the specified spot for update. At the end of the model transaction, its
	 * features will be recomputed, and other edge and track features that depends
	 * on it will be as well.
	 * <p>
	 * For the model update to happen correctly and listeners to be notified
	 * properly, a call to this method must happen within a transaction, as in:
	 *
	...
	private void flushUpdate() {
		 ...
		 * spots have been removed that causes edges to be removes, we already know
		 * about it). We do NOT recompute tracks if spots have been added: they will not
		 * result in new tracks made of single spots.
		 */

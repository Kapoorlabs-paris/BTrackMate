...
import fiji.plugin.globalbtrackmate.features.spot.SpotAnalyzer;
...
public class SpotFeatureCalculator extends MultiThreadedBenchmarkAlgorithm implements Cancelable {
	...
	/**
	 * Calculates the spot features configured in the {@link Settings} for all the
	 * spots of this model,
	 * <p>
	 * Features are calculated for each spot, using their location, and the raw
	 * image. Since a {@link SpotAnalyzer} can compute more than a feature at once,
	 * spots might received more data than required.
	 */
	@Override
	 ...
	 * but only for the spots in the specified collection. Features are calculated
	 * for each spot, using their location, and the raw image.
	 */
	 ...
	 *
	 * @param toCompute         the spots to compute.
	 * @param analyzerFactories the analyzer factories to use for computation.
	 * @param doLogIt           whether we should report progress to the user.
	 */
	private void computeSpotFeaturesAgent(final SpotCollection toCompute,
			final List<SpotAnalyzerFactoryBase<?>> analyzerFactories, final boolean doLogIt) {
		isCanceled = false;
		cancelReason = null;

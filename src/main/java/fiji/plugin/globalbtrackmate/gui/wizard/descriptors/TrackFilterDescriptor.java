...
public class TrackFilterDescriptor extends WizardPanelDescriptor {
	...
	public TrackFilterDescriptor(final TrackMate trackmate, final List<FeatureFilter> filters,
		...
		final FilterGuiPanel component = new FilterGuiPanel(trackmate.getModel(), trackmate.getSettings(),
				TrackMateObject.TRACKS, filters, TrackBranchingAnalyzer.NUMBER_SPOTS, featureSelector);
		...
		return new Runnable() {
			...
			public void run() {
				...
				try {
					...
					/*
					 * We have some tracks so we need to compute spot features will we render them.
					 */
					logger.log("\n");
					// Calculate features

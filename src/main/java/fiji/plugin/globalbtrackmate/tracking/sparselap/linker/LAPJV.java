...
public class LAPJV implements OutputAlgorithm<int[]>, Benchmark {
	...
	/**
	 * Instantiates a new Jonker-Volgenant algorithm for the specified sparse cost
	 * matrix.
	 *
	 * @param cm the cost matrix of the linear assignment problem to solve.
	 */
	...
	public boolean checkInput() {
		if (cm.nRows > cm.nCols) {
			errorMessage = BASE_ERROR_MESSAGE
					+ "This solver converges only if the cost matrix has more rows than column. Found " + cm.nRows
					+ " rows and " + cm.nCols + " columns.";
			return false;
		}
	...
	/**
	 * Returns JVS results as row assignments. The row <code>i</code> is associated
	 * to the column <code>x[i]</code> in the cost matrix.
	 *

...
import static fiji.plugin.globalbtrackmate.tracking.overlap.OverlapTrackerFactory.BASE_ERROR_MESSAGE;
...
public class OverlapTracker extends MultiThreadedBenchmarkAlgorithm implements SpotTracker, Cancelable {
	...
	public OverlapTracker(final SpotCollection spots, final IoUCalculation method, final double minIoU,
	...
	public boolean process() {
		...
		Map<Spot, Polygon2D> sourceGeometries = createGeometry(spots.iterable(sourceFrame, true), method,
				enlargeFactor);
		...
		while (frameIterator.hasNext()) {
			...
			final Map<Spot, Polygon2D> targetGeometries = createGeometry(spots.iterable(targetFrame, true), method,
					enlargeFactor);
	...
	private static Map<Spot, Polygon2D> createGeometry(final Iterable<Spot> spots, final IoUCalculation method,
			final double scale) {

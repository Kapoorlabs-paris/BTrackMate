...
import fiji.plugin.globalbtrackmate.detection.DetectionUtils;
import fiji.plugin.globalbtrackmate.detection.DetectorKeys;
import fiji.plugin.globalbtrackmate.detection.LabeImageDetectorFactory;
import fiji.plugin.globalbtrackmate.detection.SpotDetectorFactoryBase;
import fiji.plugin.globalbtrackmate.features.FeatureAnalyzer;
...
import fiji.plugin.globalbtrackmate.tracking.sparselap.SparseLAPTrackerFactory;
import ij.ImagePlus;
import ij.gui.Roi;
...
public class Settings {
	 ...
	 * {@link fiji.plugin.globalbtrackmate.visualization.TrackMateModelView} as a GUI
	 * target.
	 */
	 ...
	 * discard found spots outside the ROI. If <code>null</code>, the whole image is
	 * considered.
	 */
	public Roi roi;
	 ...
	 * {@link fiji.plugin.globalbtrackmate.detection.SpotDetector} for each target frame.
	 */
	...
	/**
	 * Settings map for {@link fiji.plugin.globalbtrackmate.detection.SpotDetector}.
	 *
	 * @see fiji.plugin.globalbtrackmate.detection.DetectorKeys
	 */
	public Map<String, Object> detectorSettings = new HashMap<>();

	/**
	 * Settings map for {@link fiji.plugin.globalbtrackmate.tracking.SpotTracker}.
	 *
	 * @see fiji.plugin.globalbtrackmate.tracking.TrackerKeys
	 */
	...
	/**
	 * The initial quality filter value that is used to clip spots of low quality
	 * from spots.
	 */
	 ...
	 * runtime. This method is a convenience for scripting, that simply discovers
	 * and adds all the analyzers it can find.
	 */
	public void addAllAnalyzers() {
		final SpotAnalyzerProvider spotAnalyzerProvider = new SpotAnalyzerProvider(
				imp == null ? 1 : imp.getNChannels());
		...
		if (imp != null && DetectionUtils.is2D(imp) && detectorFactory != null && detectorFactory.has2Dsegmentation()) {
			final SpotMorphologyAnalyzerProvider spotMorphologyAnalyzerProvider = new SpotMorphologyAnalyzerProvider(
					imp.getNChannels());
	 ...
	 * parameters are not mean to be used for all cases and need to be tuned for the
	 * actual tracking problem.
	 */
	...
	/**
	 * Returns a copy of the list of {@link SpotAnalyzerFactory}s configured in this
	 ...
	 * configured.
	 *
	 * @param spotAnalyzer the
	 *                     {@link fiji.plugin.globalbtrackmate.features.spot.SpotAnalyzer}
	 ...
	 *
	 * @param index        index at which the analyzer is to be inserted.
	 * @param spotAnalyzer the
	 *                     {@link fiji.plugin.globalbtrackmate.features.spot.SpotAnalyzer}
	 ...
	 * @return true if the specified {@link SpotAnalyzerFactory} was in the list and
	 *         was removed.
	 */
	 ...
	 * @param index        index at which the analyzer is to be inserted.
	 *
	 * @param edgeAnalyzer the {@link EdgeAnalyzer} to add, at the specified index
	 *                     in the list.
	 */
	 ...
	 * configured.
	 *
	 * @param trackAnalyzer the {@link TrackAnalyzer} to add, at the end of the
	 *                      list.
	 */
	 ...
	 * @param index         index at which the analyzer is to be inserted.
	 *
	 * @param trackAnalyzer the {@link TrackAnalyzer} to add, at the specified index
	 *                      in the list.
	 */
	...
	public static final void prettyPrintFeatureAnalyzer(final List<? extends FeatureAnalyzer> analyzers,

...
import static fiji.plugin.globalbtrackmate.Spot.FRAME;
...
import fiji.plugin.globalbtrackmate.detection.MaskUtils;
...
import fiji.plugin.globalbtrackmate.detection.SpotGlobalDetector;
import fiji.plugin.globalbtrackmate.detection.SpotGlobalDetectorFactory;
import fiji.plugin.globalbtrackmate.features.EdgeFeatureCalculator;
import fiji.plugin.globalbtrackmate.features.FeatureFilter;
import fiji.plugin.globalbtrackmate.features.SpotFeatureCalculator;
import fiji.plugin.globalbtrackmate.features.TrackFeatureCalculator;
...
public class TrackMate implements Benchmark, MultiThreaded, Algorithm, Named, Cancelable {
	 ...
	 * image. Features to be calculated and analyzers are taken from the settings
	 * field of this object.
	 *
	 * @param doLogIt if <code>true</code>, the {@link Logger} of the model will be
	 *                notified.
	 ...
	 *
	 * @param doLogIt if <code>true</code>, messages will be sent to the logger.
	 ...
	 * <p>
	 * This method links all the selected spots from the thresholding part using the
	 * selected tracking algorithm. This tracking process will generate a graph
	 * (more precisely a {@link org.jgrapht.graph.SimpleWeightedGraph}) made of the
	 * spot election for its vertices, and edges representing the links.
	 * <p>
	 ...
	 * source image and the detector settings and execute the detection process for
	...
	public <T extends Type<T>> boolean execDetection() {
		isCanceled = false;
		cancelReason = null;
		cancelables.clear();
		...
		if (settings.roi != null) {
			...
			for (int frame = settings.tstart; frame <= settings.tend; frame++) {
				for (final Spot spot : rawSpots.iterable(frame, false)) {
					...
					if (settings.roi.contains((int) Math.round(spot.getFeature(Spot.POSITION_X) / calibration[0]),
	...
	private boolean processFrameByFrame(final SpotDetectorFactory factory, final ImgPlus img, final Logger logger) {
		...
		/*
		 * Fine tune multi-threading: If we have 10 threads and 15 frames to process, we
		 * process 10 frames at once, and allocate 1 thread per frame. But if we have 10
		 * threads and 2 frames, we process the 2 frames at once, and allocate 5 threads
		 * per frame if we can.
		 */
		...
		logger.log("Detection processes " + ((nSimultaneousFrames > 1) ? (nSimultaneousFrames + " frames") : "1 frame")
				+ " simultaneously and allocates "
				+ ((threadsPerFrame > 1) ? (threadsPerFrame + " threads") : "1 thread") + " per frame.\n");
				...
				final Callable<Boolean> callable = new Callable<Boolean>() {
					...
					public Boolean call() throws Exception {
						...
						if (detector.checkInput() && detector.process()) {
							...
							/*
							 * Special case: if we have a single column image, then the detectors internally
							 * dealt with a single line image. We need to permute back the X & Y coordinates
							 * if it's the case.
							 */
							if (img.dimension(0) < 2 && zindex < 0) {
								for (final Spot spot : spotsThisFrame) {
	 ...
	 * {@link Spot#QUALITY} value for each spot it creates, so it is possible to set
	 * up an initial filtering on this feature, prior to any other operation.
	 * <p>
	 * This method simply takes all the detected spots, and discard those whose
	 * quality value is below the threshold set by
	 * {@link Settings#initialSpotFilterValue}. The spot field is overwritten, and
	 ...
	 *
	 * @param doLogIt if <code>true</code>, will send a message to the model logger.
	 * @return <code>true</code> if the computation completed without errors.
	 */

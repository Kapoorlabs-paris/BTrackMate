...
public class DefaultCostMatrixCreator<K extends Comparable<K>, J extends Comparable<J>>
	...
	public DefaultCostMatrixCreator(final List<K> rows, final List<J> cols, final double[] costs,
			final double alternativeCostFactor, final double percentile) {
		this.rows = rows;
		this.cols = cols;
		this.costs = costs;
	...
	public boolean checkInput() {
		...
		if (rows.size() != cols.size()) {
			errorMessage = BASE_ERROR_MESSAGE + "Row and column lists do not have the same number of elements. Found "
					+ rows.size() + " and " + cols.size() + ".";
			return false;
		}
		if (rows.size() != costs.length) {
			errorMessage = BASE_ERROR_MESSAGE
					+ "Row list and cost array do not have the same number of elements. Found " + rows.size() + " and "
					+ costs.length + ".";
			return false;
		}
		if (alternativeCostFactor <= 0) {
			errorMessage = BASE_ERROR_MESSAGE + "The alternative cost factor must be greater than 0. Was: "
					+ alternativeCostFactor + ".";
			return false;
		}
		if (percentile < 0 || percentile > 1) {
			errorMessage = BASE_ERROR_MESSAGE + "The percentile must no be smaller than 0 or greater than 1. Was: "
					+ percentile;
			return false;
		}
		return true;

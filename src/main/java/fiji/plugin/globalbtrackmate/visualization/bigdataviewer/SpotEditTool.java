package fiji.plugin.globalbtrackmate.visualization.bigdataviewer;
...
import fiji.plugin.globalbtrackmate.detection.semiauto.SemiAutoTracker;
...
public class SpotEditTool extends AbstractTool
	...
	HashMap<ImagePlus, BigDataViewerDisplayer> displayers = new HashMap<>();
	...
	/**
	 * Register the given {@link BigDataViewerDisplayer}. If this method id not
	 * called, the tool will not respond.
	 */
	public void register(final ImagePlus lImp, final BigDataViewerDisplayer displayer) {
		if (DEBUG)
		...
		if (DEBUG) {
			System.out.println("[SpotEditTool] Registering " + lImp + " and " + displayer + "."
	...
	public void mouseClicked(final MouseEvent e) {
		...
		final BigDataViewerDisplayer displayer = displayers.get(lImp);
		if (DEBUG) {
		...
		case 2: {
			...
			if (null == editedSpot) {
				...
				if (DEBUG)
					System.out.println(
							"[SpotEditTool] mouseClicked: Set " + editedSpot + " as editing spot for this imp.");

			} else {
				// We leave editing mode
				if (DEBUG)
					System.out.println("[SpotEditTool] mouseClicked: Got " + editedSpot
							+ " as editing spot for this imp, leaving editing mode.");

				/*
				 * A hack: we update the current z and t of the edited spot to the current one,
				 * because it is not updated otherwise: there is no way to listen to slice
				 * change
				 */
				...
				/*
				 * If we are in auto-link mode, we create an edge with spot in selection, if
				 * there is just one and if it is in a previous frame
				 */
				if (autolinkingmode) {
	...
	public void keyPressed(final KeyEvent e) {
		...
		case KeyEvent.VK_A: {
			...
			if (null == editedSpot) {
				...
				if (e.isShiftDown()) {
					...
					final double radius = (null != previousRadius) ? previousRadius : FALL_BACK_RADIUS;
		...
		case KeyEvent.VK_E: {
			...
			if (null == editedSpot) {
				...
				final double newRadius = (e.isShiftDown()) ? radius + factor * dx * COARSE_STEP
				...
				try {
					...
					logger.log(String.format(Locale.US,
							"Changed spot " + target + " radius to %.1f " + model.getSpaceUnits() + ".\n", radius));
				} finally {
					model.endUpdate();
				}
			...
			if (e.isShiftDown()) {
				...
				if (currentFrame > 0) {
					...
					final HashSet<String> featuresKey = new HashSet<>(
							spots.iterator(currentFrame - 1, true).next().getFeatures().keySet());
				...
				if (selectedSpots.size() == 2) {
					...
					if (model.getTrackModel().containsEdge(source, target)) {
						...
						if (tt != ts) {
							...
							/*
							 * To emulate a kind of automatic linking, we put the last spot to the
							 * selection, so several spots can be tracked in a row without having to
							 * de-select one
							 */
	...
	private Spot makeSpot(final ImagePlus lImp, BigDataViewerDisplayer displayer, final ImageCanvas canvas,
			Point mouseLocation) {
		if (displayer == null) {
		...
		final double[] calibration = TMUtils.getSpatialCalibration(lImp);
		return new Spot((-0.5d + canvas.offScreenXD(mouseLocation.x)) * calibration[0],
				(-0.5d + canvas.offScreenYD(mouseLocation.y)) * calibration[1], (lImp.getSlice() - 1) * calibration[2],
				FALL_BACK_RADIUS, -1d);
	}
	...
	private void updateStatusBar(final Spot spot, final String units) {
		...
		if (null == spot.getName() || spot.getName().equals(""))
			statusString = String.format(Locale.US, "Spot ID%d, x = %.1f, y = %.1f, z = %.1f, r = %.1f %s", spot.ID(),
			...
			statusString = String.format(Locale.US, "Spot %s, x = %.1f, y = %.1f, z = %.1f, r = %.1f %s",

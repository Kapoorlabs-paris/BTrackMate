...
public class SortedDepthFirstIterator<V, E> extends AbstractGraphIterator<V, E> {
	...
	/**
	 * Sentinel object. Unfortunately, we can't use null, because ArrayDeque won't
	 * accept those. And we don't want to rely on the caller to provide a sentinel
	 * object for us. So we have to play typecasting games.
	 */
	...
	private static enum VisitColor {
		...
		/**
		 * Vertex has been returned via iterator, but we're not done with all of its
		 * out-edges yet.
		 */
		GRAY,
	...
	/**
	 * Creates a new iterator for the specified graph. Iteration will start at the
	 * specified start vertex. If the specified start vertex is <code>

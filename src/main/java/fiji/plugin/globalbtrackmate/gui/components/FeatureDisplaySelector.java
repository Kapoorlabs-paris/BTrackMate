...
import static fiji.plugin.globalbtrackmate.features.FeatureUtils.collectFeatureKeys;
import static fiji.plugin.globalbtrackmate.gui.Fonts.SMALL_FONT;
import static fiji.plugin.globalbtrackmate.gui.displaysettings.DisplaySettings.TrackMateObject.DEFAULT;
import static fiji.plugin.globalbtrackmate.gui.displaysettings.DisplaySettings.TrackMateObject.EDGES;
import static fiji.plugin.globalbtrackmate.gui.displaysettings.DisplaySettings.TrackMateObject.SPOTS;
import static fiji.plugin.globalbtrackmate.gui.displaysettings.DisplaySettings.TrackMateObject.TRACKS;
...
public class FeatureDisplaySelector {
	...
	private static final List<String> FEATURES_WITHOUT_MIN_MAX = Arrays
			.asList(new String[] { FeatureUtils.USE_UNIFORM_COLOR_KEY, TrackIndexAnalyzer.TRACK_INDEX,
					ManualEdgeColorAnalyzer.FEATURE, ManualSpotColorAnalyzerFactory.FEATURE });
	 ...
	 */
	public static final CategoryJComboBox<TrackMateObject, String> createComboBoxSelector(final Model model,
		...
		final CategoryJComboBox<TrackMateObject, String> cb = new CategoryJComboBox<>(features, featureNames,
				categoryNames);
		...
		if (null != model)
			model.addModelChangeListener((event) -> {
				if (event.getEventID() == ModelChangeEvent.FEATURES_COMPUTED) {
					final LinkedHashMap<TrackMateObject, Collection<String>> features2 = new LinkedHashMap<>(
							categoriesIn.size());

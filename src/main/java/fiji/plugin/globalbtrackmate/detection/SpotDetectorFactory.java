...
public interface SpotDetectorFactory<T extends RealType<T> & NativeType<T>> extends SpotDetectorFactoryBase<T> {
	...
	/**
	 * Returns a new {@link SpotDetector} configured to operate on the given target
	 * frame. This factory must be first given the {@link ImgPlus} and the settings
	 * map, through the {@link #setTarget(ImgPlus, Map)} method.
	 *
	 * @param interval the interval that determines the region in the source image
	 *                 to operate on. This must <b>not</b> have a dimension for time

...
public interface SpotDetectorFactoryBase<T extends RealType<T> & NativeType<T>> extends TrackMateModule {
	 ...
	 * <p>
	 * Also checks the validity of the given settings map for this factory. If check
	 * fails, return <code>false</code>, an error message can be obtained through
	 * {@link #getErrorMessage()}.
	 *
	 * @param img      the {@link ImgPlus} to operate on, possible 5D.
	 * @param settings the settings map, must be suitable for this detector factory.
	 * @return <code>false</code> is the given settings map is not suitable for this
	 *         detector factory.
	 ...
	 * <p>
	 * Only parameters specific to the specific detector factory are marshalled. The
	 ...
	 * @param settings the map to update. Is cleared prior to updating, so that it
	 ...
	 *
	 * @param settings the current settings, used to get info to display on the GUI
	 *                 panel.
	 * @param model    the current model, used to get info to display on the GUI
	 *                 panel.
	 */
	public ConfigurationPanel getDetectorConfigurationPanel(final Settings settings, final Model model);

	/**
	 * Returns a new default settings map suitable for the target detector. Settings
	...
	/**
	 * Return <code>true</code> for the detectors that can provide a spot with a 2D
	 * {@link SpotRoi} when they operate on 2D images.
	 * <p>
	 * This flag may be used by clients to exploit the fact that the spots created
	 * with this detector will have a contour that can be used <i>e.g.</i> to
	 * compute morphological features. The default is <code>false</code>, indicating
	 * that this detector provides spots as a X, Y, Z, radius tuple.
	 * 

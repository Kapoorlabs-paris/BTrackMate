...
import static fiji.plugin.globalbtrackmate.gui.Icons.SELECT_TRACK_ICON;
import static fiji.plugin.globalbtrackmate.gui.Icons.SELECT_TRACK_ICON_DOWNWARDS;
import static fiji.plugin.globalbtrackmate.gui.Icons.SELECT_TRACK_ICON_UPWARDS;
import static fiji.plugin.globalbtrackmate.gui.Icons.SPOT_ICON_64x64;
import static fiji.plugin.globalbtrackmate.gui.Icons.TRACK_ICON;
import static fiji.plugin.globalbtrackmate.gui.Icons.TRACK_ICON_64x64;
...
import fiji.plugin.globalbtrackmate.util.ModelTools;
import ij.ImagePlus;
...
public class SpotEditToolConfigPanel extends JFrame {
	...
	public SpotEditToolConfigPanel(final SpotEditTool parent) {
		...
		lblQualityThreshold.setToolTipText("<html>" + "The fraction of the initial spot quality <br>"
		...
		lblDistanceTolerance.setToolTipText("<html>" + "The maximal distance above which found spots are rejected, <br>"
		...
		lblNFrames.setToolTipText(
				"<html>How many frames to process at max. <br/>Make it 0 or negative for no limit.</html>");
		...
		lblSelectTrackUpward.setToolTipText(
				"<html>" + "Select the whole tracks selected spots <br>" + "belong to, backward in time.</html>");
		...
		lblSelectTrackDown.setToolTipText(
				"<html>" + "Select the whole tracks selected spots <br>" + "belong to, forward in time.</html>");
			...
			public void log(final String message, final Color color) {
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						...
						final AttributeSet aset = sc.addAttribute(SimpleAttributeSet.EMPTY, StyleConstants.Foreground,
								color);
	...
	private void selectTrackDownward() {
		...
		if (imp == null)
			return;
		final BigDataViewerDisplayer displayer = parent.displayers.get(imp);
		if (null == displayer)
			return;

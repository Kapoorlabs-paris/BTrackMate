		...
		for (int i = frames.first(); i < frames.last(); i++) {
			...
			final Future<Void> future = executors.submit(new Callable<Void>() {
				...
				public Void call() throws Exception {
					...
					final NearestNeighborFlagSearchOnKDTree<Spot> search = new NearestNeighborFlagSearchOnKDTree<>(
							tree);

					/*
					 * For each spot in the source frame, find its nearest neighbor in the target
					 * frame.
					 */
					...
					while (sourceIt.hasNext()) {
						...
						/*
						 * The closest we could find is too far. We skip this source spot and do not
						 * create a link
						 */
						...
						/*
						 * Everything is ok. This node is free and below max dist. We create a link and

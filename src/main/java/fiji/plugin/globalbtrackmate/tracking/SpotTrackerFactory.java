...
public interface SpotTrackerFactory extends TrackMateModule {
	...
	/**
	 * Instantiates and returns a new {@link SpotTracker} configured to operate on
	 * the specified {@link SpotCollection}, using the specified settins map.
	 *
	 * @param spots    the {@link SpotCollection} containing the spots to track.
	 * @param settings the settings map configuring the tracker.
	 ...
	 *
	 * @param model the model that will be modified by the target tracker.
	 ...
	 * <p>
	 * Only parameters specific to the concrete tracker factory are marshalled. The
	 ...
	 * <p>
	 * Concretely: the the specific settings map for the targeted tracker is updated
	 * from the element.
	 *
	...
	/**
	 * A utility method that builds a string representation of a settings map owing
	 * to the currently selected tracker in this provider.
	 *
	 * @param sm the map to echo.
	...
	/**
	 * Checks the validity of the given settings map for the tracker. The validity
	 * check is strict: we check that all needed parameters are here and are of the
	 * right class, and that there is no extra unwanted parameters.
	 *
	 * @return true if the settings map can be used with the target factory. If not,
	 *         check {@link #getErrorMessage()}
	 */

package fiji.plugin.globalbtrackmate.tracking.sparselap.linker;
...
public class SparseCostMatrix {
	...
	/**
	 * The linear array storing the column index of each non infinite-cost. Column
	 * indices are stored adjacently row by row, and for each row, are stored in
	 * ascending order.
	 */
	final int[] kk;
	 ...
	 * </ol>
	 * These two arrays must be arranged row by row, starting with the first one.
	 * And in each row, the columns must be sorted in increasing order (to
	 * facilitate index search). Also, each row must have at least one non-infinte
	 * cost. If not, an {@link IllegalArgumentException} is thrown.
	 * <ol start="3">
	 ...
	 * </ol>
	 * 
	 * @param cc     the cost array.
	 * @param kk     the column index of each cost.
	 * @param number the number of element for each row.
	 * @throws IllegalArgumentException if the cost and column arrays are not of the
	 *                                  same size, if the column array is not sorted
	 *                                  row by row, of if one row has 0 non-infinite
	 *                                  costs.
	 */
	public SparseCostMatrix(final double[] cc, final int[] kk, final int[] number, final int nCols) {
		...
		if (cc.length != kk.length) {
			throw new IllegalArgumentException("Cost and column indices arrays must have the same length. Found "
					+ cc.length + " and " + kk.length + ".");
		}
		...
		for (int i = 1; i < nRows; i++) {
			if (number[i] == 0) {
				throw new IllegalArgumentException(
						"All the rows must have at least one cost. Row " + i + " have none.");
			}
		...
		for (int i = 0; i < nRows; i++) {
			...
			for (int j = start[i]; j < start[i] + number[i]; j++) {
				final int k = kk[j];
				if (k >= nCols) {
					throw new IllegalArgumentException("At line " + i
							+ ", the column indices array contains a column index (" + k
							+ ") that is larger than or equal to the declared number of column (" + nCols + ").");
				}
				colHistogram[k]++;
				if (k <= previousK) {
					throw new IllegalArgumentException(
							"The column indices array must be sorted within each row. The column elements at line " + i
		...
		for (int j = 0; j < colHistogram.length; j++) {
			if (colHistogram[j] == 0) {
				throw new IllegalArgumentException(
						"All the columns must have at least one cost. The column " + j + " has none.");
			}
		}
	}
	...
	/**
	 * Computes the total cost for an assignment specified by row. It is supposed
	 * that row <code>i</code> is assigned to column <code>rowAssignment[i]</code>.
	 * 
	 * @param rowAssignment the assignment, specified by row.
	...
	/**
	 * Returns the value stored by this matrix at the specified row and column. If a
	 * value is not present in the sparse matrix, the specified missing value is
	 * returned.
	 * 
	 * @param i            the row.
	 * @param j            the column.
	 * @param missingValue what to return if the sparse matrix does not store a
	...
	/**
	 * Returns the vertical concatenation of this matrix with the specified one. So
	 * that if this matrix is A and the specified matrix is B, you get
	 * 
	 * <pre>
	 * -----
	 * | A |
	 ...
	 * </pre>
	 * 
	 * @param B the matrix to concatenate this matrix with
	 * @return a new sparse matrix.
	 * @throws IllegalArgumentException if B does not have the same number of
	 *                                  columns as this matrix.
	 */
	public final SparseCostMatrix vcat(final SparseCostMatrix B) {
		if (nCols != B.nCols) {
			throw new IllegalArgumentException("Matrices A & B do not have the same number of columns. Found " + nCols
					+ " and " + B.nCols + " respectively.");
		}
	...
	/**
	 * Returns the horizontal concatenation of this matrix with the specified one.
	 ...
	 * @param B the matrix to concatenate this matrix with
	 * @return a new sparse matrix.
	 * @throws IllegalArgumentException if B does not have the same number of rows
	 *                                  as this matrix.
	 */
	public final SparseCostMatrix hcat(final SparseCostMatrix B) {
		if (nRows != B.nRows) {
			throw new IllegalArgumentException("Matrices A & B do not have the same number of rows. Found " + nRows
					+ " and " + B.nRows + " respectively.");
		}
	 ...
	 * 
	 * @param value the value to write in this matrix.
	 */

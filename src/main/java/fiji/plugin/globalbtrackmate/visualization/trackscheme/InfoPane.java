...
import fiji.plugin.globalbtrackmate.util.OnRequestUpdater.Refreshable;
...
public class InfoPane extends JPanel implements SelectionChangeListener {
	...
	/**
	 * A copy of the last spot collection highlighted in this infopane, sorted by
	 * frame order.
	 */
	 ...
	 *
	 * @param model          the {@link Model} from which the spot collection is
	 *                       taken.
	 * @param selectionModel the {@link SelectionModel} from which we read what to
	 *                       show in the table.
	 */
	...
	private void update() {
		/*
		 * Sort using a list; TreeSet does not allow several identical frames, which is
		 * likely to happen.
		 */
		...
		final DefaultTableModel dm = new DefaultTableModel() { // Un-editable model
			@Override
	...
	private void init() {
		...
		final SmallFeaturePlotSelectionPanel featureSelectionPanel = new SmallFeaturePlotSelectionPanel("T",
				spotFeatures, spotFeatureNames,

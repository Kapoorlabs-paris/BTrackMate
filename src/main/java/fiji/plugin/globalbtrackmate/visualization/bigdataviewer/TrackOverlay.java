...
public class TrackOverlay extends Roi {
	...
	public final synchronized void drawOverlay(final Graphics g) {
		...
		final int trackDisplayDepth = displaySettings.isFadeTracks() ? displaySettings.getFadeTrackRange()
				: 1_000_000_000;
		...
		g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
				displaySettings.getUseAntialiasing() ? RenderingHints.VALUE_ANTIALIAS_ON
						: RenderingHints.VALUE_ANTIALIAS_OFF);

		// Color generator.
		final FeatureColorGenerator<DefaultWeightedEdge> colorGenerator = FeatureUtils.createTrackColorGenerator(model,
				displaySettings);
		...
		case SELECTION_ONLY: {
			for (final DefaultWeightedEdge edge : highlight) {
				...
				final float transparency = (float) (1
						- Math.abs((double) sourceFrame - currentFrame) / trackDisplayDepth);
		...
		case FULL: {
			for (final Integer trackID : filteredTrackKeys) {
				...
				for (final DefaultWeightedEdge edge : track) {
					...
					final double zs = source.getFeature(Spot.POSITION_Z).doubleValue();
					final double zt = target.getFeature(Spot.POSITION_Z).doubleValue();
					if (doLimitDrawingDepth && Math.abs(zs - zslice) > drawingDepth
							&& Math.abs(zt - zslice) > drawingDepth)
						continue;
	...
	private static final boolean isOnClip(final Spot source, final Spot target, final double minx, final double miny,
			final double maxx, final double maxy, final double[] calibration) {
	...
	private static final boolean segmentsCross(final double x0, final double y0, final double x1, final double y1,
	...
	protected void drawEdge(final Graphics2D g2d, final Spot source, final Spot target, final int xcorner,
			final int ycorner, final double magnification, final float transparency) {
		// Find x & y in physical coordinates
		final double x0i = source.getFeature(Spot.POSITION_X);
		final double y0i = source.getFeature(Spot.POSITION_Y);
		final double x1i = target.getFeature(Spot.POSITION_X);
	...
	protected void drawEdge(final Graphics2D g2d, final Spot source, final Spot target, final int xcorner,
			final int ycorner, final double magnification) {
		// Find x & y in physical coordinates
		final double x0i = source.getFeature(Spot.POSITION_X);
		final double y0i = source.getFeature(Spot.POSITION_Y);
		final double x1i = target.getFeature(Spot.POSITION_X);

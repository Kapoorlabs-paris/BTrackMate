...
import static fiji.plugin.globalbtrackmate.visualization.trackscheme.TrackScheme.DEFAULT_CELL_HEIGHT;
import static fiji.plugin.globalbtrackmate.visualization.trackscheme.TrackScheme.DEFAULT_CELL_WIDTH;
import static fiji.plugin.globalbtrackmate.visualization.trackscheme.TrackScheme.X_COLUMN_SIZE;
import static fiji.plugin.globalbtrackmate.visualization.trackscheme.TrackScheme.Y_COLUMN_SIZE;
...
import fiji.plugin.globalbtrackmate.graph.ConvexBranchesDecomposition;
import fiji.plugin.globalbtrackmate.graph.GraphUtils;
...
import fiji.plugin.globalbtrackmate.graph.ConvexBranchesDecomposition.TrackBranchDecomposition;
	...
	public void execute(final Object lParent) {
		...
		try {
			...
			for (final Integer trackID : model.getTrackModel().trackIDs(true)) { // will be sorted by track name
				 ...
				 * That is: if a track has at least a spot that has more than one predecessor.
				 * If we have to deal with such a case, we revert to the old, slow scheme.
				 */
				...
				if (isTree) {
					...
					final SortedDepthFirstIterator<Spot, DefaultWeightedEdge> iterator = model.getTrackModel()
							.getSortedDepthFirstIterator(first, Spot.nameComparator, false);
					...
					final DepthFirstIterator<List<Spot>, DefaultEdge> depthFirstIterator = new DepthFirstIterator<>(
							branchGraph);

...
import static fiji.plugin.globalbtrackmate.gui.Icons.MAGNIFIER_ICON;
...
import fiji.plugin.globalbtrackmate.visualization.trackscheme.SpotIconGrabber;
...
public class ExtractTrackStackAction extends AbstractTMAction {
	...
	public static final String INFO_TEXT = "<html> " + "Generate a stack of images taken from the track "
			+ "that joins two selected spots. " + "<p> "
			...
			+ "them. A path is then found that joins them and the stack is extracted " + "from this path." + "<p> "
			...
			+ "spot in the track, and whether to capture a 2D or 3D stack over time. " + "All channels are captured. "
			+ "</html>";
	...
	public void execute(final TrackMate btrackmate, final SelectionModel selectionModel,
		...
		if (nspots != 2) {
			if (nspots == 1) {
				...
				// Get stack.
				final ImagePlus imp = trackStack(btrackmate, spot, do3d, logger);
				imp.show();
			...
			// Get stack.
			final ImagePlus imp = trackStack(btrackmate, start1, end1, do3d, logger);
			imp.show();
			imp.setZ(imp.getNSlices() / 2 + 1);
			imp.resetDisplayRange();
		}
	...
	public static final ImagePlus trackStack(final TrackMate btrackmate, final Spot spot, final boolean do3d,
			final Logger logger) {
		final Model model = btrackmate.getModel();
		final Integer trackID = model.getTrackModel().trackIDOf(spot);
		final List<Spot> spots = new ArrayList<>(model.getTrackModel().trackSpots(trackID));
		...
		final Spot start = spots.get(0);
		final Spot end = spots.get(spots.size() - 1);
		return trackStack(btrackmate, start, end, do3d, logger);
	}

	public static final ImagePlus trackStack(final TrackMate btrackmate, final Spot start, final Spot end,
			final boolean do3d, final Logger logger) {
		final Model model = btrackmate.getModel();
		final Spot start1;
		final Spot end1;
		if (start.getFeature(Spot.POSITION_T) > end.getFeature(Spot.POSITION_T)) {
		...
		return trackStack(btrackmate.getSettings(), path, radius, do3d, logger);
	}

	@SuppressWarnings({ "unchecked", "rawtypes" })
	public static final ImagePlus trackStack(final Settings settings, final List<Spot> path, final double radius,
			final boolean do3d, final Logger logger) {
			...
			for (int c = 0; c < nChannels; c++) {
				...
				if (do3d) {
					...
					for (int i = 0; i < crop.dimension(2); i++) {
						final ImageProcessor processor = ImageJFunctions
								.wrap(Views.hyperSlice(crop, 2, i), crop.toString()).getProcessor();

...
public class MaskUtils {
	 ...
	 * 
	 * @param <T>        the type of the input image. Must be real, scalar.
	 * @param input      the input image.
	 * @param interval   the interval in the input image to analyze.
	 * @param threshold  the threshold to apply to the input image.
	 * @param numThreads how many threads to use for multithreaded computation.
	 * @return a new label image.
	 */
	public static final <T extends RealType<T>> ImgLabeling<Integer, IntType> toLabeling(
			final RandomAccessible<T> input, final Interval interval, final double threshold, final int numThreads) {
		// Crop.
		...
		final ExecutorService executorService = numThreads > 1 ? Executors.newFixedThreadPool(numThreads)
				: Executors.newSingleThreadExecutor();

		ConnectedComponents.labelAllConnectedComponents(bitMask, labeling, MaskUtils.labelGenerator(), se,
				executorService);
	...
	/**
	 * Creates spots from a grayscale image, thresholded to create a mask. A spot is
	 * created for each connected-component of the mask, with a size that matches
	 * the mask size.
	 * 
	 * @param <T>         the type of the input image. Must be real, scalar.
	 * @param input       the input image.
	 * @param interval    the interval in the input image to analyze.
	 * @param calibration the physical calibration.
	 * @param threshold   the threshold to apply to the input image.
	 * @param numThreads  how many threads to use for multithreaded computation.
	 * @return a list of spots, without ROI.
	 */
	public static <T extends RealType<T>> List<Spot> fromThreshold(final RandomAccessible<T> input,
			final Interval interval, final double[] calibration, final double threshold, final int numThreads) {
	 ...
	 * 
	 * @param <R>         the type that backs-up the labeling.
	 * @param labeling    the labeling, must be zero-min.
	 * @param interval    the interval, used to reposition the spots from the
	 *                    zero-min labeling to the proper coordinates.
	 * @param calibration the physical calibration.
	 * @return a list of spots, without ROI.
	 */
	public static <R extends IntegerType<R>> List<Spot> fromLabeling(final ImgLabeling<Integer, R> labeling,
		...
		while (iterator.hasNext()) {
			...
			final double radius = (labeling.numDimensions() == 2) ? Math.sqrt(volume / Math.PI)
	...
	public static Pair<SpotCollection, HashMap<Integer, ArrayList<Spot>>> fromSimpleCSV(
			HashMap<Integer, ArrayList<Cellobject>> CSV, final int ndims, final double[] calibration) {
		// Parse each component.
	 ...
	 * Creates spots from a grayscale image, thresholded to create a mask. A spot is
	 * created for each connected-component of the mask, with a size that matches
	 * the mask size. The quality of the spots is read from another image, by taking
	 * the max pixel value of this image with the ROI.
	 * 
	 * @param <T>          the type of the input image. Must be real, scalar.
	 * @param input        the input image.
	 * @param interval     the interval in the input image to analyze.
	 ...
	 * @param threshold    the threshold to apply to the input image.
	 * @param numThreads   how many threads to use for multithreaded computation.
	 * @param qualityImage the image in which to read the quality value.
	 * @return a list of spots, without ROI.
	 */
	public static <T extends RealType<T>, R extends RealType<R>> List<Spot> fromThreshold(
			final RandomAccessible<T> input, final Interval interval, final double[] calibration,
			final double threshold, final int numThreads, final RandomAccessibleInterval<R> qualityImage) {
		// Get labeling from mask.
		final ImgLabeling<Integer, IntType> labeling = toLabeling(input, interval, threshold, numThreads);
	 ...
	 * thresholded to create a mask. A spot is created for each connected-component
	 * of the mask, with a size that matches the mask size. The quality of the spots
	 * is read from another image, by taking the max pixel value of this image with
	 * the ROI.
	 * 
	 * @param <T>          the type of the input image. Must be real, scalar.
	 * @param <S>          the type of the quality image. Must be real, scalar.
	 * @param input        the input image. Must be 2D.
	 * @param interval     the interval in the input image to analyze.
	 * @param calibration  the physical calibration.
	 * @param threshold    the threshold to apply to the input image.
	 * @param simplify     if <code>true</code> the polygon will be post-processed
	 *                     to be smoother and contain less points.
	 * @param numThreads   how many threads to use for multithreaded computation.
	 * @param qualityImage the image in which to read the quality value.
	...
	public static final <T extends RealType<T>, S extends NumericType<S>> List<Spot> fromThresholdWithROI(
			final RandomAccessible<T> input, final Interval interval, final double[] calibration,
			final double threshold, final boolean simplify, final int numThreads,
			final RandomAccessibleInterval<S> qualityImage) {
		if (input.numDimensions() != 2)
			throw new IllegalArgumentException(
					"Can only process 2D images with this method, but got " + input.numDimensions() + "D.");
	 ...
	 * value is read from a secondary image, byt taking the max value in each ROI.
	 * 
	 * @param <R>          the type that backs-up the labeling.
	 * @param <S>          the type of the quality image. Must be real, scalar.
	 * @param labeling     the labeling, must be zero-min and 2D..
	 * @param interval     the interval, used to reposition the spots from the
	 *                     zero-min labeling to the proper coordinates.
	 * @param calibration  the physical calibration.
	 * @param simplify     if <code>true</code> the polygon will be post-processed
	...
	public static <R extends IntegerType<R>, S extends NumericType<S>> List<Spot> fromLabelingWithROI(
			final ImgLabeling<Integer, R> labeling, final Interval interval, final double[] calibration,
			final boolean simplify, final RandomAccessibleInterval<S> qualityImage) {
		if (labeling.numDimensions() != 2)
			throw new IllegalArgumentException(
					"Can only process 2D images with this method, but got " + labeling.numDimensions() + "D.");
		...
		final ImagePlus qualityImp = (null == qualityImage) ? null : ImageJFunctions.wrap(qualityImage, "QualityImage");
	...
	private static final void douglasPeucker(final List<double[]> list, final int s, final int e, final double epsilon,
	...
	/**
	 * Parse a 2D mask and return a list of polygons for the external contours of
	 * white objects.
	 * <p>
	 ...
	 */
	private static final <T extends BooleanType<T>> List<Polygon> maskToPolygons(
				...
				if (thisRow[x + 1]) { // i.e., pixel (x,y) is selected
					if (!prevRow[x + 1]) {
						...
						if (outline[x] == null) {
							...
							if (x < w - 1 && y < h && x != xAfterLowerRightCorner && !thisRow[x + 2]
									&& prevRow[x + 2]) { // at lower right corner & next pxl deselected
								outline[x] = null;
...
//								polygons.add( outline[ x ].getPolygon() );
					...
					if (prevRow[x + 1]) {
						...
						if (outline[x] == null) {
							...
							if (x < w - 1 && y < h && x != xAfterLowerRightCorner && thisRow[x + 2]
									&& !prevRow[x + 2]) { // at lower right corner & next pxl selected
								outline[x] = null;
								// outline[x+1] unchanged
								outline[x + 1].append(x + 1, y);
								xAfterLowerRightCorner = x + 1;
	...
	/**
	 * This class implements a Cartesian polygon in progress. The edges are supposed
	 * to be parallel to the x or y axis. It is implemented as a deque to be able to
	 * add points to both sides.
	 */
	private static class Outline {
		...
		/**
		 * Default extra (spare) space when enlarging arrays (similar performance with
		 * 6-20)
		 */
		...
		public void prepend(final Outline o) {
			...
			if (size <= o.reserved - o.last && oSize > first) { /*
																 * We don't have enough space in our own array but in
																 * that of 'o' so append our own data to that of 'o'
																 */
		...
		public String toString() {
			String res = "[first:" + first + ",last:" + last + ",reserved:" + reserved + ":";
			if (last > x.length)

...
import static fiji.plugin.globalbtrackmate.visualization.trackscheme.TrackScheme.DEFAULT_COLOR;
...
public class TrackSchemeStylist {
	...
	public synchronized void updateEdgeStyle(final Collection<mxCell> edges) {
		final FeatureColorGenerator<DefaultWeightedEdge> trackColorGenerator = FeatureUtils
				.createTrackColorGenerator(model, displaySettings);
		...
		try {
			for (final mxCell cell : edges) {
				...
				style = mxStyleUtils.setStyle(style, mxConstants.STYLE_STROKEWIDTH,
						Float.toString((float) displaySettings.getLineThickness()));
	...
	public void updateVertexStyle(final Collection<mxCell> vertices) {
		...
		final FeatureColorGenerator<Spot> spotColorGenerator = FeatureUtils.createSpotColorGenerator(model,
				displaySettings);
		final Color missingValueColor = displaySettings.getMissingValueColor();

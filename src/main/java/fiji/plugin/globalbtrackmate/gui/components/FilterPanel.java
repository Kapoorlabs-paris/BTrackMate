...
public class FilterPanel extends javax.swing.JPanel {
	...
	public FilterPanel(final Map<String, String> keyNames, final Function<String, double[]> valueCollector,
		...
		final ComboBoxModel<String> cmbboxFeatureNameModel = new DefaultComboBoxModel<>(
				keyNames.keySet().toArray(new String[] {}));
		...
		this.add(cmbboxFeatureKeys, new GridBagConstraints(0, 0, 3, 1, 0.0, 0.0, GridBagConstraints.CENTER,
		...
		final JFreeChart chart = ChartFactory.createHistogram(null, null, null, dataset, PlotOrientation.VERTICAL,
				false, false, false);
		...
		intervalMarker = new IntervalMarker(0, 0, new Color(0.3f, 0.5f, 0.8f), new BasicStroke(), new Color(0, 0, 0.5f),
		...
		this.add(chartPanel, new GridBagConstraints(0, 1, 3, 1, 0.0, 0.0, GridBagConstraints.CENTER,
		...
		this.add(btnAutoThreshold, new GridBagConstraints(2, 2, 1, 1, 0.0, 0.0, GridBagConstraints.EAST,
		...
		this.add(rdbtnAbove, new GridBagConstraints(0, 2, 1, 1, 0.0, 0.0, GridBagConstraints.WEST,
		...
		this.add(rdbtnBelow, new GridBagConstraints(1, 2, 1, 1, 0.0, 0.0, GridBagConstraints.CENTER,
	 ...
	 * be notified when a change happens to the threshold displayed by this panel,
	 ...
	 * representation as he types, then converting the string to a double after a
	 * wait time. The number typed is used to set the threshold in the chart panel.
	 *
	 * @author Jean-Yves Tinevez
	 */
	private final class MyKeyListener implements KeyListener {
		...
		public void keyPressed(final KeyEvent e) {
			...
			if (e.getKeyCode() == KeyEvent.VK_LEFT || e.getKeyCode() == KeyEvent.VK_KP_LEFT) {
				threshold -= (e.isControlDown() ? SLOW_INCREASE_FACTOR : INCREASE_FACTOR)
						* plot.getDomainAxis().getRange().getLength();
				redrawThresholdMarker();
				return;

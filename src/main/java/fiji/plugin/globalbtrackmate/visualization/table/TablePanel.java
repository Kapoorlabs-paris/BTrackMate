...
import fiji.plugin.globalbtrackmate.gui.displaysettings.ColorIcon;
...
public class TablePanel<O> {
	...
	public TablePanel(final Iterable<O> objects, final List<String> features,
			final BiFunction<O, String, Double> featureFun, final Map<String, String> featureNames,
			final Map<String, String> featureShortNames, final Map<String, String> featureUnits,
			final Map<String, Boolean> isInts, final Map<String, String> infoTexts,
			final Supplier<FeatureColorGenerator<O>> colorSupplier, final Function<O, String> labelGenerator,
			final BiConsumer<O, String> labelSetter) {
		this(objects, features, featureFun, featureNames, featureShortNames, featureUnits, isInts, infoTexts,
				colorSupplier, labelGenerator, labelSetter, null, null);
	}
	...
	public TablePanel(final Iterable<O> objects, final List<String> features,
			...
			final Map<String, Boolean> isInts, final Map<String, String> infoTexts,
			final Supplier<FeatureColorGenerator<O>> colorSupplier, final Function<O, String> labelGenerator,
			final BiConsumer<O, String> labelSetter, final String manualColorFeature,
	...
	/**
	 * The panel in which the table is displayed. This is the component to add to
	 * client UI.
	 * 
			...
			for (int r = 0; r < nRows; r++) {
				...
				for (int col = 0; col < nCols; col++) {
					...
					if (null == obj)
						...
						content[col] = String.format("r=%d;g=%d;b=%d", color.getRed(), color.getGreen(),
								color.getBlue());
					} else
	...
	private class MyTableCellRenderer extends DefaultTableCellRenderer {
		...
		public MyTableCellRenderer() {
			this.normalBorder = ((JLabel) super.getTableCellRendererComponent(table, "", false, false, 0, 0))
					.getBorder();
	...
	private class MyColorEditor extends AbstractCellEditor implements TableCellEditor {
		...
		public Component getTableCellEditorComponent(final JTable table, final Object value, final boolean isSelected,
			...
			button.addActionListener(e -> {
				...
				final JDialog d = JColorChooser.createDialog(button, "Choose a color", true, colorChooser,
						new ActionListener() {
							@Override

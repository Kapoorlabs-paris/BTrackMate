...
import static fiji.plugin.globalbtrackmate.gui.Icons.LABEL_IMG_ICON;
import static fiji.plugin.globalbtrackmate.gui.Icons.TRACKMATE_ICON;
...
import fiji.plugin.globalbtrackmate.util.SpotUtil;
...
public class LabelImgExporter extends AbstractTMAction {
	...
	public static final String INFO_TEXT = "<html>" + "This action creates a label image from the tracking results. "
			+ "<p> " + "A new 16-bit image is generated, of same dimension and size that "
			...
			+ "larger than the last trackID in the dataset. " + "<p> " + "Only visible spots are painted. " + "</html>";
	...
	public void execute(final TrackMate btrackmate, final SelectionModel selectionModel,
		...
		if (gui != null) {
			...
			final int userInput = JOptionPane.showConfirmDialog(gui, panel, "Export to label image",
	...
	/**
	 * Creates a new label {@link ImagePlus} where the spots of the specified model
	 * are painted as ellipsoids taken from their shape, with their track ID as
	 * pixel value.
	 *
	 * @param btrackmate        the btrackmate instance from which we takes the
	 *                          spots to paint. The label image will have the same
	 *                          calibration, name and dimension from the input image
	 *                          stored in the btrackmate settings. The output label
	 *                          image will have the same size that of this input
	 *                          image, except for the number of channels, which will
	 *                          be 1.
	 * @param exportSpotsAsDots if <code>true</code>, spots will be painted as
	 *                          single dots instead of ellipsoids.
	 * @param exportTracksOnly  if <code>true</code>, only the spots belonging to
	 *                          visible tracks will be painted. If
	 *                          <code>false</code>, spots not belonging to a track
	 *                          will be painted with a unique ID, different from the
	 *                          track IDs and different for each spot.
	 *
	 * @return a new {@link ImagePlus}.
	 */
	public static final ImagePlus createLabelImagePlus(final TrackMate btrackmate, final boolean exportSpotsAsDots,
			final boolean exportTracksOnly) {
	 ...
	 *                          will be painted with a unique ID, different from the
	 *                          track IDs and different for each spot.
	 * @param logger            a {@link Logger} instance, to report progress of the
	 *                          export process.
	 *
	 * @return a new {@link ImagePlus}.
	 */
	public static final ImagePlus createLabelImagePlus(final TrackMate btrackmate, final boolean exportSpotsAsDots,
			final boolean exportTracksOnly, final Logger logger) {
		return createLabelImagePlus(btrackmate.getModel(), btrackmate.getSettings().imp, exportSpotsAsDots,
				exportTracksOnly, logger);
	}
	 ...
	 * pixel value.
	 *
	 * @param model             the model from which we takes the spots to paint.
	 * @param imp               a source image to read calibration, name and
	 *                          dimension from. The output label image will have the
	 *                          same size that of this source image, except for the
	 *                          number of channels, which will be 1.
	 * @param exportSpotsAsDots if <code>true</code>, spots will be painted as
	 *                          single dots instead of ellipsoids.
	 * @param exportTracksOnly  if <code>true</code>, only the spots belonging to
	 *                          visible tracks will be painted. If
	...
	public static final ImagePlus createLabelImagePlus(final Model model, final ImagePlus imp,
			final boolean exportSpotsAsDots, final boolean exportTracksOnly) {
	 ...
	 */
	public static final ImagePlus createLabelImagePlus(final Model model, final ImagePlus imp,
			final boolean exportSpotsAsDots, final boolean exportTracksOnly, final Logger logger) {
		final int[] dimensions = imp.getDimensions();
		final int[] dims = new int[] { dimensions[0], dimensions[1], dimensions[3], dimensions[4] };
		final double[] calibration = new double[] { imp.getCalibration().pixelWidth, imp.getCalibration().pixelHeight,
				imp.getCalibration().pixelDepth, imp.getCalibration().frameInterval };
		final ImagePlus lblImp = createLabelImagePlus(model, dims, calibration, exportSpotsAsDots, exportTracksOnly,
				logger);
	 ...
	 *
	 * @param model             the model from which we takes the spots to paint.
	 * @param dimensions        the desired dimensions of the output image (width,
	 *                          height, nZSlices, nFrames) as a 4 element int array.
	 *                          Spots outside these dimensions are ignored.
	 * @param exportSpotsAsDots if <code>true</code>, spots will be painted as
	 *                          single dots instead of ellipsoids.
	 * @param exportTracksOnly  if <code>true</code>, only the spots belonging to
	 *                          visible tracks will be painted. If
	...
	public static final ImagePlus createLabelImagePlus(final Model model, final int[] dimensions,
			final double[] calibration, final boolean exportSpotsAsDots, final boolean exportTracksOnly) {
		return createLabelImagePlus(model, dimensions, calibration, exportSpotsAsDots, exportTracksOnly,
				Logger.VOID_LOGGER);
	}
	...
	public static final ImagePlus createLabelImagePlus(final Model model, final int[] dimensions,
		...
		final ImagePlus lblImp = ImageJFunctions.wrap(
				createLabelImg(model, dims, calibration, exportSpotsAsDots, exportTracksOnly, logger), "LblImage");
	...
	/**
	 * Creates a new label {@link Img} of {@link UnsignedShortType} where the spots
	 * of the specified model are painted as ellipsoids taken from their shape, with
	 * their track ID as pixel value.
	 *
	 * @param model             the model from which we takes the spots to paint.
	 * @param dimensions        the desired dimensions of the output image (width,
	 *                          height, nZSlices, nFrames) as a 4 element int array.
	...
	public static final Img<UnsignedShortType> createLabelImg(final Model model, final long[] dimensions,
		...
		final Img<UnsignedShortType> lblImg = Util.getArrayOrCellImgFactory(targetSize, new UnsignedShortType())
				.create(targetSize);
		...
		for (int frame = 0; frame < dimensions[3]; frame++) {
			...
			final SpotWriter spotWriter = exportSpotsAsDots ? new SpotAsDotWriter(imgCT) : new SpotRoiWriter(imgCT);

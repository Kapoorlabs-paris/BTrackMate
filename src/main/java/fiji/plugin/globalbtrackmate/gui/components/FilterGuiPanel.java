...
import static fiji.plugin.globalbtrackmate.features.FeatureUtils.collectFeatureValues;
import static fiji.plugin.globalbtrackmate.features.FeatureUtils.nObjects;
...
import static fiji.plugin.globalbtrackmate.gui.Icons.ADD_ICON;
import static fiji.plugin.globalbtrackmate.gui.Icons.REMOVE_ICON;
...
import fiji.plugin.globalbtrackmate.util.OnRequestUpdater;
...
public class FilterGuiPanel extends JPanel implements ChangeListener {
	...
	public FilterGuiPanel(final Model model, final Settings settings, final TrackMateObject target,
			final List<FeatureFilter> filters, final String defaultFeature,
	...
	public void addFilterPanel(final FeatureFilter filter) {
		...
		final Function<String, double[]> valueCollector = (featureKey) -> collectFeatureValues(featureKey, target,
	...
	private String guessNextFeature() {
		...
		if (featureFilters.isEmpty())
			return (defaultFeature == null || !featureNames.keySet().contains(defaultFeature)) ? it.next()
					: defaultFeature;

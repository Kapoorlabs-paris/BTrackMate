...
public class SpotFitEllipseAnalyzer<T extends RealType<T>> extends AbstractSpotFeatureAnalyzer<T> {
	 ...
	 * @return
	 *         <p>
	 *         6-element array, {a b c d f g}, which are the algebraic parameters of
	 *         the fitting ellipse: <i>ax</i><sup>2</sup> + 2 <i>bxy</i> +
	 *         <i>cy</i><sup>2</sup> +2<i>dx</i> + 2<i>fy</i> + <i>g</i> = 0. The
	 *         vector <b>A</b> represented in the array is normed, so that
	 *         ||<b>A</b>||=1.
	 *         </p>
	 *
	 * @see
	 ...
	 * We always have a > b. theta in radians measure the angle of the ellipse long
	 * axis with the x axis, in radians, and positive means counter-clockwise.
	 * 
	 * Formulas from https://en.wikipedia.org/wiki/Ellipse#In_Cartesian_coordinates
	 */
	private static final double[] quadraticToCartesian(final double[] Q) {
		...
		final double term1 = 2 * (A * E * E + C * D * D - B * D * E + (B * B - 4 * A * C) * F);

...
public class OverlapTrackerFactory implements SpotTrackerFactory {
	...
	/**
	 * The key to the parameter that stores scale factor parameter. The scale factor
	 * allows for enlarging (>1) or shrinking (<1) the spot shapes before computing
	 * their IoU. Values are strictly positive {@link Double}s.
	 */
	public static final String KEY_SCALE_FACTOR = "SCALE_FACTOR";

	/**
	 * The key to the parameter that stores the minimal IoU below which links are
	 * not created. Values are strictly positive {@link Double}s.
	 */
	public static final String KEY_MIN_IOU = "MIN_IOU";

	/**
	 * The key to the parameter that stores how the IoU should be calculated. There
	 * are two methods specified as strings: {@value #FAST_CALCULATION} and
	 * {@value #PRECISE_CALCULATION}.
	 */
	...
	public static final String TRACKER_INFO_TEXT = "<html> "
			+ "This tracker is a simple extension of the Intersection - over - Union (IoU) tracker. " + "<p> " + "<p> "
			+ "It generates links between spots whose shapes overlap between consecutive frames. "
			+ "When several spots are eligible as a source for a target, the one with the largest IoU " + "is chosen."
			+ "<p> " + "<p> "
			...
			+ "as 2D objects. The Z dimension is ignored. " + "</html>";

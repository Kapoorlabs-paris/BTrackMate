package fiji.plugin.globalbtrackmate.features.edges;
...
public class DirectionalChangeAnalyzer extends AbstractEdgeAnalyzer {
	...
	protected void analyze(final DefaultWeightedEdge edge, final Model model) {
		...
		/*
		 * Rate of directional change. We need to fetch the predecessor edge, via the
		 * source.
		 */
		...
		/*
		 * We work only if there is only one predecessor. The directional change is
		 * anyway not defined in case of branching.
		 */
		if (nPredecessors != 1) {
	...
	private static final double dotProduct(final double dx1, final double dy1, final double dz1, final double dx2,
			final double dy2, final double dz2) {
		return dx1 * dx2 + dy1 * dy2 + dz1 * dz2;
	}

	private static final void crossProduct(final double dx1, final double dy1, final double dz1, final double dx2,
			final double dy2, final double dz2, final double[] out) {

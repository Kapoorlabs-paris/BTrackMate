...
import fiji.plugin.globalbtrackmate.detection.util.MedianFilter2D;
...
public class DetectionUtils {
	 ...
	 * thread. It executes the detection in one frame only and writes the results in
	 * the specified model object.
	 * 
	 * @param model            the model to write detection results in.
	 * @param settings         the settings to use for the image input and the ROI
	 *                         input.
	 * @param detectorFactory  the detector factory to use for detection.
	 * @param detectorSettings the settings for the detection, specific to the
	 *                         detector factory/
	 * @param frame            the frame (0-based) to execute the detection in.
	 * @param logger           a logger to write results and error messages to.
	 * @param buttonEnabler    a consumer that will receive <code>false</code> at
	 *                         the beginning of the preview, and <code>true</code>
	 *                         at its end. Can be used to disable GUI elements.
	 */
	public static final void preview(final Model model, final Settings settings,
			final SpotDetectorFactoryBase<?> detectorFactory, final Map<String, Object> detectorSettings,
			final int frame, final Logger logger, final Consumer<Boolean> buttonEnabler) {
	...
	/**
	 * Returns <code>true</code> if the specified image is 2D. It can have multiple
	 * channels and multiple time-points; this method only looks at whether several
	 * Z-slices can be found.
	 * 
	 * @param img the image.
	 * @return
	 */
	public static final boolean is2D(final ImgPlus<?> img) {
		return img.dimensionIndex(Axes.Z) < 0 || img.dimension(img.dimensionIndex(Axes.Z)) <= 1;
	}
	...
	/**
	 * Creates a laplacian of gaussian (LoG) kernel tuned for blobs with a radius
	 * specified <b>using calibrated units</b>. The specified calibration is used to
	 * determine the dimensionality of the kernel and to map it on a pixel grid.
	 * 
	 * @param radius      the blob radius (in image unit).
	 * @param nDims       the dimensionality of the desired kernel. Must be 1, 2 or
	 *                    3.
	 * @param calibration the pixel sizes, specified as <code>double[]</code> array.
	 * @return a new image containing the LoG kernel.
	 */
	public static final Img<FloatType> createLoGKernel(final double radius, final int nDims,
		...
		while (c.hasNext()) {
			...
			/*
			 * LoG normalization factor, so that the filtered peak have the maximal value
			 * for spots that have the size this kernel is tuned to. With this value, the
			 * peak value will be of the same order of magnitude than the raw spot (if it
			 * has the right size). This value also ensures that if the image has its
			 * calibration changed, one will retrieve the same peak value than before
			 * scaling. However, I (JYT) could not derive the exact formula if the image is
			 * scaled differently across X, Y and Z.
			 */
	 ...
	 *         coordinates (0, 0).
	 */
	public static final <T extends RealType<T>> Img<FloatType> copyToFloatImg(final RandomAccessible<T> img,
			final Interval interval, final ImgFactory<FloatType> factory) {
	...
	/**
	 * Returns a new {@link Interval}, built by squeezing out singleton dimensions
	 ...
	 */
	public static final <R extends RealType<R> & NativeType<R>> Img<R> applyMedianFilter(
	...
	public static final List<Spot> findLocalMaxima(final RandomAccessibleInterval<FloatType> source,
			final double threshold, final double[] calibration, final double radius,
			final boolean doSubPixelLocalization, final int numThreads) {
		/*
		 * Find maxima.
		 */
		...
		final IntervalView<FloatType> dogWithBorder = Views.interval(Views.extendMirrorSingle(source),
				Intervals.expand(source, 1));
		final ExecutorService service = Executors.newFixedThreadPool(numThreads);
		List<Point> peaks;
		try {
			peaks = LocalExtrema.findLocalExtrema(dogWithBorder, localNeighborhoodCheck, new RectangleShape(1, true),
					service, numThreads);
		...
		if (doSubPixelLocalization) {
			...
			/*
			 * Deal with different dimensionality manually. Profound comment: this is the
			 * proof that this part of the code is sloppy. ImgLib2 is supposed to be
			 * dimension-generic. I just did not use properly here.
			 */
	 ...
	 */
	public static final <T extends Type<T>> RandomAccessibleInterval<T> prepareFrameImg(final ImgPlus<T> img,
		...
		if (singleTimePoint.dimensionIndex(Axes.CHANNEL) < 0)
			...
			singleChannel = ImgPlusViews.hyperSlice(singleTimePoint, singleTimePoint.dimensionIndex(Axes.CHANNEL),
					channel);

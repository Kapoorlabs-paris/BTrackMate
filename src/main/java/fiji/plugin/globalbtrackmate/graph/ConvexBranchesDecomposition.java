...
public class ConvexBranchesDecomposition implements Algorithm, Benchmark {
	 ...
	 *
	 * @param model             the {@link Model} from which tracks are to be split.
	 *                          Only tracks marked visible will be processed.
	 * @param forbidMiddleLinks specifies whether we enforce links between branches
	 *                          to be between an end point of a branch and a start
	 *                          point of another branch. If <code>true</code>, links
	 *                          will only reach for these spots. If
	 *                          <code>false</code>, a link can target a spot within
	 *                          a branch, which can lead to fewer and longer
	 *                          branches.
	 * @param forbidGaps        specifies whether we forbid gaps in tracks. If
	 *                          <code>true</code>, a track containing a gap
	 *                          (detections missing in at least 1 consecutive
	 *                          frames) will be split in 2 branches. If
	...
	/**
	 * Creates a new track splitter. Links between spots from within branches and
	 * gaps within convex branches are forbidden.
	 *
	 * @param model the {@link Model} from which tracks are to be split. Only tracks
	...
	public boolean checkInput() {
		...
		for (final DefaultWeightedEdge edge : tm.edgeSet()) {
			...
			if (source.diffTo(target, Spot.FRAME) == 0d) {
				errorMessage = BASE_ERROR_MSG + "Cannot deal with links between two spots in the same frame (" + source
						+ " & " + target + ").\n";
				return false;
			}
		}
	 ...
	 * track in a model.
	 *
	 * @param trackID           the ID of the track to decompose.
	 * @param tm                the {@link TrackModel} in which the track is stored.
	 * @param neighborIndex     a {@link TimeDirectedNeighborIndex} needed to
	 *                          quickly retrieve neighbors in the mother graph.
	 * @param forbidMiddleLinks if <code>true</code>, the decomposition will include
	 *                          branches where only the first and last spots may
	 *                          have more than one predecessor and one successor
	 *                          respectively. If <code>false</code>, some spots
	 *                          inside a branch may be a fusion or splitting point.
	 *                          This leads to fewer and longer branches.
	 * @param forbidGaps        if <code>true</code>, two neighbor spots in a branch
	 *                          will be separated by exactly one frame. If
	 *                          <code>false</code>, branches will include gaps.
	...
	public static final TrackBranchDecomposition processTrack(final Integer trackID, final TrackModel tm,
			final TimeDirectedNeighborIndex neighborIndex, final boolean forbidMiddleLinks, final boolean forbidGaps) {
	 ...
	 * 
	 * @param branchDecomposition the convex branch decomposition to transform.
	 ...
	 */
	public static final SimpleDirectedGraph<List<Spot>, DefaultEdge> buildBranchGraph(
		...
		for (final List<Spot> link : links) {
			...
			if (targetBranch == null) {
				/*
				 * We could not find this link's target in the map of first spots. Most likely
				 * this means that the link targets a middle spot, because the branch
				 * decomposition authorized it. So we have to find it...
				 */
				for (final List<Spot> branch : branches) {
	 ...
	 * ordered in the list by increasing frame number, and that two consecutive spot
	 * are separated by exactly one frame.
	 *
	 ...
	 * is ensured that the first element of all links is the last spot of a branch,
	 * and the second element of this link is the first spot of another branch.
	 * Otherwise, a link can target a spot within a branch.
	 *
